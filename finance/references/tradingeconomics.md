...
1. Start a managed browser with `browser-tools start --task finance` if one is not already running. Keep the reported port and owner token for the helper commands.
...
6. Stop the managed browser with `browser-tools stop --clean --port <reported port> --owner-token <token>` after the work if you started it for the task.
