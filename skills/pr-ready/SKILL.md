...
Run the bundled state command from the PR worktree:
...
or review change. Never infer readiness from memory or from an earlier green run.
...
names, review-thread result, tests actually run, and the state command's final `VERDICT` verbatim.
