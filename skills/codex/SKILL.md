---
name: codex
description: Drive the OpenAI Codex CLI non-interactively as a second agent with full session lifecycle control (create, get status, converse across turns, stop, delete), run tracked code review sessions (start in background, poll status, fetch the report, continue the same review thread with follow-up prompts), delegate implementation tasks to Codex as tracked background exec workers with liveness verdicts (running, wedged, stalled, dead) a coordinator agent can poll, and run a persistent Codex MCP server for continuous multi-turn conversation such as adversarial discussion, review synthesis, or design debate with full context retention across turns. Use this skill whenever the user mentions codex, a codex review, an adversarial or second-opinion review by another model, talking to codex, a back-and-forth or continuous conversation with codex, launching codex in MCP mode, delegating or scheduling implementation work to codex, checking whether a codex run is alive or progressing, managing or cleaning up codex sessions, or orchestrating codex as a non-interactive worker or reviewer from automation or workflows.
