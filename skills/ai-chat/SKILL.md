...
compatibility: "Requires @rezkam/browser-tools from package.json, macOS Chrome, Node.js 22.19+, installed npm dependencies, and logged-in browser sessions for selected providers."
---
...
- Reuse: a command can adopt a still-healthy owned browser at start. It reads the private AI Chat browser state file, validates Browser Tools managed state, owner token, and copied profile presence, then reconnects to that browser.
- Close: every provider closes the owned browser when its command finishes, including on failure. Nothing is left running for a later command to inherit. The one exception is `--submit-only`, which hands a live browser to the `--final` command that reads the result, and that command closes it.
