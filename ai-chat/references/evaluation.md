...
| Gemini | WebUI model aliases `flash`, `thinking`, `extended-thinking`, and `reasoning`, task defaults for quick and reasoning, unknown model rejection | `gemini-3.6-flash`, `gemini-3.6-flash-extended-thinking` | Gemini model id from the WebUI API result | Live checks require Google cookies. Backend error `1052` fallback must be explicit. Deep research is not a stable AI Chat profile |
